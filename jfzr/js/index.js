var Event = {
    eventId: 1059,
    eventAid: 'pc_event_jfzr201509',
    eventName: 'JFZR201508',
...
//        });
        ...
        $("#bind_game_area").unbind("click").bind("click", function () {
...
//        this.loginShow();
    ...
    event: function () {
        ...
        // 弹层
        $("#btn_get_mainGift").bind('click', function () {
            showDialog.show({
                id: 'select_area',
                bgcolor: '#000',
                opacity: 60,
    ...
    loginShow: function () {
        $.ajax({
            type: 'post',
            url: 'http://ac.qq.com/event/7xi201508/action.php?action=show',
            dataType: 'json',
            success: function (data) {
                if (data.status == -99) {
                    AC.Page.showLogin(location.pathname);
                } else {
                    $('#unlogin').hide().siblings().show();
                    $('#login_qq_span').text(data.nick)
                }
            }
        });
    },
    navAnchor: function () {
        ...
        $('.nav-link-sub').bind('click', function () {
            var _thisClass = $(this).attr('href'),
                    _thisOffset = $('.' + _thisClass).offset().top;

            if ($(this).hasClass('nav-link-top')) {
                _thisOffset = 0;
            }
            $('body, html').animate({
    ...
    chapterTab: function () {
        ...
        btn.bind('click', function () {
            ...
            if (_thisIndex == 0) {
                list.children().eq(0).show().siblings().hide();
            }
            else if (_thisIndex == 2) {
                list.children().last().show().siblings().hide();
            } else {
                return false;
            }
        });
        ...
        item.bind('click', function () {
            ...
            list.children().eq(_thisIndex).show().siblings().hide();
        });
    }
}
